...
Принять сырой текст СМС от банка. Сохраняет в БД и ставит на фоновую обработку в Celery.
...
**Поля ответа:**
- `status` — всегда `"queued"` (обработка асинхронна)
- `transaction_id` — ID созданной записи в БД

**Что происходит:**
1. СМС сохраняется в БД (в т.ч. статус `is_parsed: false`)
2. Возвращается сразу же (< 1 сек)
3. Параллельно в Celery запускается фоновая задача `process_sms_task`:
   - Отправляется в Ollama для парсинга
   - Результаты парсинга сохраняются в транзакцию
   - Запускается финансовая логика (привязка к периоду, проверка лимитов)
   - Если настроена VK интеграция, отправляется уведомление

**Примечание:** Парсинг происходит асинхронно (обычно 5–30 сек). Статус может быть проверен через `GET /api/finance/summary` (список открытых периодов обновляется после завершения обработки).
